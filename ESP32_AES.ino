...
void doBlockTest(unsigned char *key, unsigned char *input, unsigned char *output, uint16_t len) {
  ...
  Serial.println("\n\nInput:");
  esp_aes_hexDump(input, len);
  uint8_t rslt = esp_aes_multiple_blocks(ESP_AES_ENCRYPT, key, input, output, len);
  Serial.println("\nOutput [encoded] [" + String(rslt) + "]:");
  esp_aes_hexDump(output, len);
  ...
  rslt = esp_aes_multiple_blocks(ESP_AES_DECRYPT, key, output, result, len);
  Serial.println("\nOutput [decoded] [" + String(rslt) + "]:");
  esp_aes_hexDump(result, len);
  Serial.print("esp_aes_block: "); Serial.print(t1 - t0); Serial.println(" microseconds");
...
void doBufferTest(unsigned char *key, unsigned char *input, unsigned char *output, unsigned char iv[16], uint16_t len) {
  ...
  Serial.println("\nesp_aes_crypt_cfb8:");
  t0 = system_get_time();
  rslt = esp_aes_crypt_cfb8(ESP_AES_ENCRYPT, len, iv, key, input, output);
  t1 = system_get_time();
  Serial.println("\nOutput [encoded] [" + String(rslt) + "]:");
  esp_aes_hexDump(output, len);
  Serial.print("esp_aes_crypt_cfb8: "); Serial.print(t1 - t0); Serial.println(" microseconds");

  memcpy(myIV, iv, 16);
  t0 = system_get_time();
  rslt = esp_aes_crypt_cfb8(ESP_AES_DECRYPT, len, iv, key, output, result);
  t1 = system_get_time();
  Serial.println("\nOutput [decoded] [" + String(rslt) + "]:");
  esp_aes_hexDump(result, len);
  Serial.print("esp_aes_crypt_cfb8: "); Serial.print(t1 - t0); Serial.println(" microseconds\n");
